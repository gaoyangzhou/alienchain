 ...
 */
package org.alienchain.api.http;
...
import org.alienchain.api.ApiHandler;
import org.alienchain.config.Config;
import org.alienchain.util.BasicAuth;
...
/**
 * HTTP handler for Alienchain API.
 */
public class HttpHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    private static final MimetypesFileTypeMap mimeTypesMap = new MimetypesFileTypeMap(
            HttpHandler.class.getResourceAsStream("/org/alienchain/api/mime.types"));
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest msg) {
        ...
        if (!checkBasicAuth(headers)) {
            ...
            resp.headers().set(HttpHeaderNames.WWW_AUTHENTICATE, "Basic realm=\"Alienchain RESTful API\"");
        ...
        if ("/".equals(path)) {
            lastContentFuture = writeStaticFile(ctx, "/org/alienchain/api/index.html");
            ...
            if (path.startsWith("/swagger-ui/")) {
                ...
                lastContentFuture = writeStaticFile(ctx, "/org/alienchain/api" + path);
            }
