...
import static org.alienchain.net.msg.p2p.NodesMessage.MAX_NODES;
...
import org.alienchain.net.msg.MessageQueue;
import org.alienchain.net.msg.MessageWrapper;
...
/**
 * Alienchain P2P message handler
 */
public class AlienchainP2pHandler extends SimpleChannelInboundHandler<Message> {

    private final static Logger logger = LoggerFactory.getLogger(AlienchainP2pHandler.class);
     ...
     * @param channel
     */
    public AlienchainP2pHandler(Channel channel, Kernel kernel) {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        if (isNewHandShakeEnabled()) {
            ...
            if (channel.isOutbound()) {
                msgQueue.sendMessage(new org.alienchain.net.msg.p2p.handshake.v1.HelloMessage(
    ...
    public void channelRead0(final ChannelHandlerContext ctx, Message msg) throws InterruptedException {
        ...
        case DISCONNECT:
            ...
            break;
        case HELLO:
            onHello((org.alienchain.net.msg.p2p.handshake.v1.HelloMessage) msg);
            break;
        case WORLD:
            onWorld((org.alienchain.net.msg.p2p.handshake.v1.WorldMessage) msg);
            break;
        case PING:
            onPing();
            break;
    ...
    protected void onHello(org.alienchain.net.msg.p2p.handshake.v1.HelloMessage msg) {
        ...
        msgQueue.sendMessage(new org.alienchain.net.msg.p2p.handshake.v1.WorldMessage(
                config.network(), config.networkVersion(), client.getPeerId(),
                client.getIp(), client.getPort(), config.getClientId(), chain.getLatestBlockNumber(),
                client.getCoinbase()));
    ...
    protected void onWorld(org.alienchain.net.msg.p2p.handshake.v1.WorldMessage msg) {
        Peer peer = msg.getPeer();
    ...
    private ReasonCode checkPeer(Peer peer, boolean newHandShake) {
        ...
        if (newHandShake && !config.network().equals(peer.getNetwork())
                || !newHandShake && Stream.of(peer.getCapabilities())
                        .noneMatch(k -> (config.network() == Network.MAINNET ? "ALX" : "SEM_TESTNET").equals(k))) {
