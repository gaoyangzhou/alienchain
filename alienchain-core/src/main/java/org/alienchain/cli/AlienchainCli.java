...
import org.alienchain.Launcher;
...
import org.alienchain.core.bip39.Language;
import org.alienchain.core.bip39.MnemonicGenerator;
...
import org.alienchain.net.filter.exception.IpFilterJsonParseException;
...
/**
 * Alienchain command line interface.
 */
public class AlienchainCli extends Launcher {

    public static final boolean HD_WALLET_ENABLED = false;

    private static final Logger logger = LoggerFactory.getLogger(AlienchainCli.class);

    public static void main(String[] args, AlienchainCli cli) {
        try {
            // check jvm version
    ...
    public static void main(String[] args) {
        main(args, new AlienchainCli());
    }

    /**
     * Creates a new Alienchain CLI instance.
     */
    public AlienchainCli() {
        SystemUtil.setLocale(getConfig().locale());

        Option helpOption = Option.builder()
                .longOpt(AlienchainOption.HELP.toString())
                .desc(CliMessages.get("PrintHelp"))
                .build();
        addOption(helpOption);

        Option versionOption = Option.builder()
                .longOpt(AlienchainOption.VERSION.toString())
                .desc(CliMessages.get("ShowVersion"))
                .build();
        addOption(versionOption);

        Option accountOption = Option.builder()
                .longOpt(AlienchainOption.ACCOUNT.toString())
        ...
        Option changePasswordOption = Option.builder()
                .longOpt(AlienchainOption.CHANGE_PASSWORD.toString()).desc(CliMessages.get("ChangeWalletPassword")).build();
        addOption(changePasswordOption);

        Option dumpPrivateKeyOption = Option.builder()
                .longOpt(AlienchainOption.DUMP_PRIVATE_KEY.toString())
        ...
        Option importPrivateKeyOption = Option.builder()
                .longOpt(AlienchainOption.IMPORT_PRIVATE_KEY.toString())
    ...
    public void start(String[] args) throws ParseException, IOException {
        ...
        if (cmd.hasOption(AlienchainOption.HELP.toString())) {
            printHelp();

        } else if (cmd.hasOption(AlienchainOption.VERSION.toString())) {
            printVersion();

        } else if (cmd.hasOption(AlienchainOption.ACCOUNT.toString())) {
            String action = cmd.getOptionValue(AlienchainOption.ACCOUNT.toString()).trim();
        ...
        } else if (cmd.hasOption(AlienchainOption.CHANGE_PASSWORD.toString())) {
            changePassword();

        } else if (cmd.hasOption(AlienchainOption.DUMP_PRIVATE_KEY.toString())) {
            dumpPrivateKey(cmd.getOptionValue(AlienchainOption.DUMP_PRIVATE_KEY.toString()).trim());

        } else if (cmd.hasOption(AlienchainOption.IMPORT_PRIVATE_KEY.toString())) {
            importPrivateKey(cmd.getOptionValue(AlienchainOption.IMPORT_PRIVATE_KEY.toString()).trim());
    ...
    protected void printHelp() {
        ...
        formatter.printHelp("./alienchain-cli.sh [options]", getOptions());
    }
