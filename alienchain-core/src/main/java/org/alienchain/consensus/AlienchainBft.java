...
import org.alienchain.consensus.AlienchainBft.Event.Type;
import org.alienchain.consensus.exception.AlienchainBftException;
...
import org.alienchain.util.ArrayUtil;
...
/**
 * Implements Alienchain BFT engine based on single-thread event model. States are
 ...
 * </ul>
 */
public class AlienchainBft implements BftManager {
    private static final Logger logger = LoggerFactory.getLogger(AlienchainBft.class);
    ...
    public AlienchainBft(Kernel kernel) {
        this.kernel = kernel;
    ...
    protected void enterCommit() {
        ...
        if (!blockHash.isPresent()) {
            throw new AlienchainBftException("Entered COMMIT state without +2/3 pre-commit votes");
        } else {
    ...
    protected Block proposeBlock() {
        ...
        AlienchainBlock alienchainBlock = new AlienchainBlock(tempHeader, config.vmBlockGasLimit());
        ...
        for (PendingManager.PendingTransaction tx : pending) {
            if (tx.transaction.getType() == TransactionType.CALL
                ...
                if (tx.transaction.getGasPrice() >= config.vmMinGasPrice()
                    ...
                    TransactionResult result = exec.execute(tx.transaction, as, ds, alienchainBlock, chain);
    ...
    protected boolean validateBlockProposal(Block block) {
        ...
        List<TransactionResult> results = exec.execute(transactions, as, ds,
                new AlienchainBlock(header, Long.MAX_VALUE), chain);
    ...
    protected void applyBlock(Block block) {
        ...
        if (number != chain.getLatestBlockNumber() + 1) {
            throw new AlienchainBftException("Applying wrong block: number = " + number);
        }
        ...
        List<TransactionResult> results = exec.execute(transactions, as, ds,
                new AlienchainBlock(block.getHeader(), Long.MAX_VALUE), chain);
        if (!block.validateResults(header, results)) {
