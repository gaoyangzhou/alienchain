...
import static org.alienchain.core.Fork.UNIFORM_DISTRIBUTION;
...
import org.alienchain.TestUtils;
...
public class AlienchainBftTest {

    private static final Logger logger = LoggerFactory.getLogger(AlienchainBftTest.class);

    @Rule
    public KernelRule kernelRule = new KernelRule(19169, 19246);
  ...
  @Test
    public void testIsPrimaryH256() {
        ...
        AlienchainBft bft = mock(AlienchainBft.class);
  ...
  @Test
    ...
    private void testIsPrimaryConsecutiveValidatorProbability(AlienchainBft bft) {
        int blocks = 100;
        int repeat = 0;
        int last = -1;
  ...
  @Test
    public void testDuplicatedTransaction() {
        ...
        kernelRule.getKernel().getBlockchain().getAccountState().adjustAvailable(from1.toAddress(), ALX.of(1000));
        ...
        AlienchainBft alienchainBFT = new AlienchainBft(kernelRule.getKernel());

        // create a tx with the same hash with tx1 from a different signer in the second
        // block
        Key from2 = new Key();
        kernelRule.getKernel().getBlockchain().getAccountState().adjustAvailable(from2.toAddress(), ALX.of(1000));
        ...
        alienchainBFT.proposal = new Proposal(new Proof(block2.getNumber(), 0), block2.getHeader(), Collections.emptyList());
        alienchainBFT.proposal.sign(from2);
        assertFalse(alienchainBFT.validateBlockProposal(block2.getHeader(), block2.getTransactions()));
    }
  ...
  @Test
    public void testFilterPendingTransactions() {
        ...
        assertTrue(alienchainBFT.getUnvalidatedTransactions(Collections.singletonList(tx1)).isEmpty());

        // other transactions should remain
        assertFalse(alienchainBFT.getUnvalidatedTransactions(Collections.singletonList(tx2)).isEmpty());
  ...
  @Test
    public void testValidateBlockCoinbaseMagic() {
        ...
        AlienchainBft alienchainBFT = new AlienchainBft(kernelRule.getKernel());
        assertFalse(alienchainBFT.validateBlockProposal(block.getHeader(), Collections.emptyList()));
    }
  ...
  @Test
    public void testValidateBlockCoinbaseProposer() {
        ...
        AlienchainBft alienchainBFT = new AlienchainBft(kernelRule.getKernel());
        alienchainBFT.proposal = new Proposal(new Proof(block.getNumber(), 0), block.getHeader(), Collections.emptyList());
        alienchainBFT.proposal.sign(new Key());
        assertFalse(alienchainBFT.validateBlockProposal(block.getHeader(), Collections.emptyList()));
    }
  ...
  @Test
    public void testValidateTransactionRecipient() {
        ...
        AlienchainBft alienchainBFT = new AlienchainBft(kernelRule.getKernel());
        alienchainBFT.proposal = new Proposal(new Proof(block.getNumber(), 0), block.getHeader(),
                Collections.singletonList(tx));
        alienchainBFT.proposal.sign(blockForger);
        assertFalse(alienchainBFT.validateBlockProposal(block.getHeader(), Collections.singletonList(tx)));
    }
  ...
  @Test
    public void testProposeBlock() {
        ...
        AlienchainBft bft = new AlienchainBft(kernelRule.getKernel());
        bft.height = number + 1;
